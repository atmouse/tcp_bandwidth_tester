...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)    {
    ...
    */
    ...
    if (!g_waiting && !syn && ack)    {
        //check incoming packets for any ack, just to determine the receiver's window size
        ...
        if (strcmp(dest_address, g_dest_address) == 0)   {
            g_total_data += size_payload;
            /*
            if (g_total_data == g_window_size)  {
            ...
            */
            ...
            printf("\n");
        } else if (strcmp(src_address, g_dest_address) == 0)    {
